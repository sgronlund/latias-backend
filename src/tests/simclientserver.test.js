...
describe("Test Suite for Server", () => {
  ...
  beforeEach((done) => {
    const tableUsers = db.prepare('CREATE TABLE IF NOT EXISTS users (username VARCHAR(255), password VARCHAR(255), email varchar(255), resetcode varchar(255))');
    const tableQuestions = db.prepare('CREATE TABLE IF NOT EXISTS questions (question varchar(255), wrong1 varchar(255), wrong2 varchar(255), wrong3 varchar(255), correct varchar(255))');
    tableUsers.run();
    tableQuestions.run();
    done();
  });
  ...
  test("Add question and check with correct answer", (done) => {
    ...
    clientSocket.emit("correctAnswer", "QUESTION", ["FALSE1","FALSE2","FALSE3","CORRECT"]);
  });
  ...
  test("Add question and check with wrong answer", (done) => {
    ...
    clientSocket.emit("wrongAnswer", "QUESTION", ["FALSE1","FALSE2","FALSE3","CORRECT"]);
  });
  ...
  test("Get existing question", (done) => {
    serverSocket.on("getQuestion", (question) => {
      expect(backend.addQuestion("QUESTION", ["FALSE","FALSE","FALSE","CORRECT"], db)).toBeTruthy();
      const getQuestion = backend.getQuestion(question, db);
      expect(getQuestion).toEqual({"correct": "CORRECT", "question": "QUESTION", "wrong1": "FALSE", "wrong2": "FALSE", "wrong3": "FALSE"});
      done();
    });
  ...
  test("StringifySeconds", (done) => {
    serverSocket.on("stringifySeconds", (week) => {
      expect(backend.stringifySeconds(week)).toBe("days: 0 hours: 15 minutes: 5 seconds: 21");
      done();
    });
